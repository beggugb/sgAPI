const sucursalService = require("../services/sucursalService.js");
const sucursalItemsService = require("../services/sucursalItemService.js");
...
const getData = (req,res) =>{       
        sucursalService.data(req.params.page,req.params.num,req.params.prop,req.params.value)
        .then((rows)=>{
...
const setUpdate = (req,res) =>{        
        ...
        .then((xrow)=>{            
            sucursalService.data(1,15,'nombre','asc')
            .then((rows)=>{
                res.status(200).send({result: rows})    
            })
        })
...
const getSearch = (req, res) =>{
  ...
  let ivalue = verifiDBNull(value)
    sucursalService.search(prop, ivalue)
    .then((rows)=>{
        res.status(200).send({result: rows})
    })
    .catch((reason)=>{
