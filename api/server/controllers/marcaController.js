const marcaService = require("../services/marcaService.js");
...
const getData = (req,res) =>{       
        marcaService.data(req.params.page,req.params.num,req.params.prop,req.params.value)
        .then((rows)=>{
...
const setUpdate = (req,res) =>{        
        ...
        .then((xrow)=>{            
            marcaService.data(1,15,'nombre','asc')
            .then((rows)=>{
                res.status(200).send({result: rows})    
            })
        })
...
const setDelete = (req,res) =>{    
        marcaService._delete(req.params.id)
        .then((xrow)=>{
...
const getList = (req,res) =>{
    marcaService.list(req.params.id)
    .then((rows)=>{
        res.status(200).send({result: rows})
    })
    .catch((reason)=>{        
...
const getSearch = (req, res) =>{
  ...
  let ivalue = verifiDBNull(value)
    marcaService.search(prop, ivalue)
    .then((rows)=>{
        res.status(200).send({result: rows})
    })
    .catch((reason)=>{
